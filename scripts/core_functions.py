...
def revcom(s):
    ...
    """
    basecomp = {'A': 'T', 'C': 'G', 'G': 'C', 'T': 'A', 'N': 'N', 'K': 'M', 'M': 'K', 'R': 'Y', 'Y': 'R', 'S': 'S',
                'W': 'W', 'B': 'V',
                'V': 'B', 'H': 'D', 'D': 'H', '-': '-'}
    letters = list(s[::-1])
    ...
    if intron_flag:  # if sequence contains both UTR(s) and intron(s)
        ...
        for idx, utr in enumerate(new_utrs):
            ...
            if new_aligned_utr != utr:
                if rev_com == True:
                    new_aligned_utr = revcom(new_aligned_utr)
        ...
        if rev_com == True:  # Now that UTR has been found, reverse complement again so correct translation input
            new_aligned = revcom(new_aligned)
    ...
    if utr_flag:  # if sequence contains both UTR(s) and introns(s), pass data to combo function
        ...
        for idx, intron in enumerate(new_introns):
            ...
            if new_aligned_intron != intron.upper():
                if rev_com == True:
                    new_aligned_intron = revcom(new_aligned_intron)
...
def remove_utr_intron_combo(translation_ref_seq, aligned, rev_com):
    ...
    for idx, removal_seq in enumerate(removal_df['sequence']):
        ...
        if new_aligned_to_remove != removal_seq:
            if rev_com == True:
                new_aligned_to_remove = revcom(new_aligned_to_remove)
...
codon_map = {'TTT': 'F', 'TTC': 'F', 'TTA': 'L', 'TTG': 'L', 'CTT': 'L', 'CTC': 'L', 'CTA': 'L', 'CTG': 'L', 'ATT': 'I',
             'ATC': 'I',
            ...
            for seq in sequence_inputs:
                if seq:  # if not empty
                    ...
                    if non_nuc_char:  # if there are any characters that are not A, C, G, or T or brackets
                        print(non_nuc_char)
                        print(
                            'Invalid character in sequence. Please make sure the sequence only contains As, Cs, Gs, and Ts.')
                        return
                    else:
            ...
            #             translation_err_flag =
                ...
                if len_last_codon == 0:
                    ...
                    if translation_input[-len_last_codon:] != last_codon[:len_last_codon]:
                        ...
                        print(
                            'Input for last_codon does not align with frame and translation_ref_seq. Please check inputs.')
                        err_flag = True

                if not err_flag:
                    print('Expected WT Translation: ',
                          translate(translation_input_list, frame, first_codon, last_codon, codon_map))
...
def get_bev_str(bev):
    ...
    INPUTS
    ------
    bev: str, BEV sample number

    OUTPUTS
    -------
    str(bev): correctly formatted BEV sample number such that single digit preceded by '00' and double digit preceded by '0'
    '''
    bev = int(bev)
    if bev < 10:
        return '00' + str(bev)
...
def check_allele_freq_filepath(filepath, bev, sg_seq):
    '''
...
def check_nuc_per_filepath(filepath, bev):
    '''
    Called from: get_path
    Function: Verifies existence of filepath generated by get_path function to retrieve 'Alleles_frequency_table_around_sgRNA'       file
    ...
    OUTPUTS
    -------
    file_loc : filepath of 'Nucleotide_percentage_table.txt' file if exists
    None if no filepath exists

    '''
    #     file_loc = filepath+'/CRISPResso_on_NGBEV_'+bev+'_'+primer+'/'+'Nucleotide_percentage_table.txt'
    file_loc = filepath + 'CRISPResso_on_' + bev + '/' + 'Nucleotide_percentage_table.txt'
    print(file_loc)
    if path.exists(file_loc):
        return file_loc
    else:
...
def get_path(bev_string_id, bev_num, filepath, sg_seq='', table='allele_freq'):
    '''
    ...
    Calls: check_allele_freq_filepath or check_nuc_per_filepath

    INPUTS
    ------
    bev_string_id :'BEV' or 'NGBEV'
    bev_num : BEV sample number
    filepath: filepath to CRISPResso folder
    sg_seq: guide sequence
    table : 'allele_freq' or 'nuc_per' depending on which notebook function is being called from

    OUTPUTS
    -------
    file_loc : filepath of 'Alleles_frequency_table_around_sgRNA_[].txt' or 'Nucleotide_percentage_table.txt' file if exists
    None if no filepath exists
    ...
    #     filepath = CRISPResso_filepath
    #     return check_filepath(filepath,bev,primer,sg_seq)
    if table == 'allele_freq':
        print(filepath, bev, sg_seq)
        return check_allele_freq_filepath(filepath, bev, sg_seq)
    elif table == 'nuc_per':
        return check_nuc_per_filepath(filepath, bev)
...
def merge_bev_file(filepath, bev, sg_seq, existing_df, cols):
    ...
    df_summed = df[['Aligned_Sequence', 'Reference_Sequence', '#Reads', '%Reads']].groupby(
        ['Aligned_Sequence', 'Reference_Sequence'], as_index=False).agg('sum')
    cols.append(str('_BEV_' + str(bev)))
    df_summed = df_summed.rename(
        columns={'#Reads': str('#Reads_BEV_' + str(bev)), '%Reads': str('%Reads_BEV_' + str(bev))})
...
def get_bev_files(bev_string_id, bev_list, crispresso_filepath):
    ...
    for bev, sg_seq in bev_list:
        ...
        filepath = get_path(bev_string_id=bev_string_id, bev_num=bev, filepath=crispresso_filepath, sg_seq=sg_seq)
        if filepath != '':
...
def get_lfc_v2(row, data_file):
    ...
    for i, bev in enumerate(bev_list):
        ...
        # Calculate LFC
        data_file['LFC_' + test + '-' + ref] = data_file['#Reads_BEV_' + test + ';lognorm'] - data_file[
            '#Reads_BEV_' + ref + ';lognorm']
...
def process_data_v2(data, bev_string_id, CRISPResso_filepath, read_count_percent_cutoff):
    ...
    for col in ['#Reads' + col for col in cols]:
        ...
        merge.loc[:, str(col + ';lognorm')] = merge[col].apply(
            lambda x: log((float(x) / float(colsum)) * 1000000 + 1, 2))

    print(['%Reads' + col for col in cols])

    # Apply read count filter based on user input
    merge.loc[:, '%read_count_filter'] = merge.apply(read_count_filter,
                                                     args=(['%Reads' + col for col in cols], read_count_percent_cutoff),
                                                     axis=1)  # less than 1% of all reads
    merge.loc[:, '#read_count_filter'] = merge.apply(read_count_filter, args=(['#Reads' + col for col in cols], 100),
                                                     axis=1)  # less than 100 reads
    ...
    if utr_flag and intron_flag:
        ...
        filtered_input_df['Aligned_Sequence'] = list(
            map(remove_utr_intron_combo, filtered_input_df['translation_ref_seq'],
                filtered_input_df['Aligned_Sequence'], filtered_input_df['rev_com']))
    ...
    # If translated_ref_seq contains only UTRs, call remove_utr function
        ...
        filtered_input_df['Aligned_Sequence'] = list(
            map(remove_utr, filtered_input_df['translation_ref_seq'], filtered_input_df['old_Aligned_Sequence'].copy(),
                filtered_input_df['rev_com']))
    ...
    # If translated_ref_seq contains only introns, call remove_introns function
        ...
        filtered_input_df['Aligned_Sequence'] = list(
            map(remove_introns, filtered_input_df['translation_ref_seq'], filtered_input_df['old_Aligned_Sequence'],
                filtered_input_df['rev_com']))
    ...
    if utr_flag or intron_flag:
        ...
        if rev_com == True:
            merge.loc[:, 'Aligned_Sequence'] = merge.loc[:, 'Aligned_Sequence'].apply(revcom)
            merge.loc[:, 'Reference_Sequence'] = merge.loc[:, 'Reference_Sequence'].apply(revcom)
        ...
        merge.loc[:, 'Translated'] = filtered_input_df['Aligned_Sequence'].apply(translate, args=(
        row['frame'], row['first_codon'], row['last_codon'], codon_map,))
    ...
    # if no introns or UTR
        ...
        merge.loc[:, 'Translated'] = merge.loc[:, 'Aligned_Sequence'].apply(translate, args=(
        row['frame'], row['first_codon'], row['last_codon'], codon_map,))
...
def run(input_file, corr_input, bev_string_id, CRISPResso_filepath, read_count_percent_cutoff, output_filepath):
    ...
    bev_string_id: str, 'BEV' or 'NGBEV'
    CRISPResso_filepath: str, filepath to folder where CRISPResso files are stored
    read_count_percent_cutoff: float, %reads cut-off used to filter out low-frequency alleles
    output_filepath: str, filepath to folder where outputs are being stored
    ...
    for sg in sg_list:
        ...
        data_dedup = data.drop_duplicates(
            subset=['sg', 'sgRNA_sequence', 'primer', 'frame', 'rev_com', 'BEV_ref', 'BEV_test'])
        ...
        for row_num, row in merge.iterrows():
            ...
            for i, aa in enumerate(translation):
                if aa == wt_translation[
                    i]:  # '_' if nt at position is the not edited (i.e. same in aligned and reference)
        ...
        if corr_input is not None:  # skip if no corr_input provided
            ...
            correlations = correlations.melt(id_vars='R1', value_vars=list(correlations).remove('R1'), var_name='R2',
                                             value_name='Pearson')
            ...
            correlations.to_csv(
                output_filepath + "corr_outputs/#read_count_filter_pearson/sg" + str(r['sg']) + '_correlations.csv')
    #         break
    return
...
def heatmaps(df, vmin, vmax, filepath, time1='Time1', time2='Time2',
    ...
    filename: str, default: 'translation_heatmap'
    ...
    for num, (i, r) in enumerate(df.iterrows()):
        ...
        for idx, row in to_plot.iterrows():
            ...
            if not np.isnan(frame):
                ...
                if guide_start == -1:
                    guide_start = wt_ref_seq.find(revcom(guide_seq))
        ...
        if row_count < num_rows:
            ...
            sns.heatmap(heatmap_df.set_index('Label'), cmap=cmap, vmin=vmin, vmax=vmax, ax=heatmap_ax, annot=True,
                        yticklabels=True, fmt='.1f')
            ...
            reads_table = table_ax.table(cellText=cell_text, colLabels=reads_df.columns, loc='center', cellLoc='center',
                                         edges='open')  # , bbox = [0,0, width, height/2.3])
...
########## NUCLEOTIDE PERCENTAGE FUNCTIONS ################

def get_bev_df(bev_list, output_name, guide, be_type, CRISPResso_filepath, bev_string_id, output_filepath):
    '''
    This function generates the file with per position percentage frequencies of each nucleotide with an offset_position that
    contains the position relative to the guide. This file is used to generate the nucleotide percentage bar plot.

    INPUTS
    ------
    bev_list: list, list of BEV sample numbers
    output_name: str, concatenated string of BEV sample numbers
    guide: str, sgRNA sequence
    be_type: str, 'ABE' or 'CBE' from user input
    CRISPResso_filepath: str, path to folder with CRISPResso outputs
    bev_string_id: str, 'BEV' or 'NGBEV' based on how samples are named
    output_filepath: str, path to folder where outputs are being stored

    OUTPUTS
    -------
    existing_df: df, dataframe with per position percentage frequencies of each nucleotide with an offset_position that
    contains the position relative to the guide

    '''
    for i, bev in enumerate(bev_list):
        filepath = CRISPResso_filepath
        #         file_loc = nb.check_nuc_per_filepath(filepath,get_bev_str(BEV))
        file_loc = get_path(bev_string_id=bev_string_id, bev_num=bev,
                            sg_seq=guide, filepath=filepath, table='nuc_per')
        ...
        data = data.rename(
            columns={'A': 'A_' + str(bev), 'C': 'C_' + str(bev), 'G': 'G_' + str(bev), 'T': 'T_' + str(bev),
                     'del': 'del_' + str(bev), 'N': 'N_' + str(bev)})
...
def make_nuc_per_plot(data, left_lim, right_lim, bevs, width, height, be_type, output_name, output_filepath):
    '''
    This function generates the file with per position percentage frequencies of each nucleotide with an offset_position that
    contains the position relative to the guide. This file is used to generate the nucleotide percentage bar plot.
    ...
    INPUTS
    ------
    data: df, output from get_bev_df
    left_lim: int, x-axis left limit
    right_lim: int, x-axis right limit
    bevs: list, list of BEV sample numbers
    width: int, plot width (figsize parameter)
    height: int, plot height (figsize parameter)
    be_type: str, 'ABE' or 'CBE'
    output_name: str, concatenated string of BEV sample numbers
    output_filepath: str, path to folder where outputs are being stored
    ...
    if be_type == 'ABE':
        order = data.loc[(data['nucleotide'] == 'C') & (data[
                                                            'BEV'] == 'avg'), 'offset_position'].tolist()  # require that 'BEV' == 'avg' just to deduplicate list
        hue_order = ['C', 'G', 'T', 'N', 'del']

    elif be_type == 'CBE':
        order = data.loc[(data['nucleotide'] == 'A') & (data[
                                                            'BEV'] == 'avg'), 'offset_position'].tolist()  # require that 'BEV' == 'avg' just to deduplicate list
    ...
    sns.barplot(x='offset_position', y='percentage', hue='nucleotide', data=data.loc[data['BEV'] == 'avg', :],
                order=order, hue_order=hue_order, palette=sns.color_palette('Set2'),
                linewidth=0)

    # Plot individual replicates as dots
    for bev in bevs:
        sns.stripplot(x='offset_position', y='percentage', hue='nucleotide', data=data.loc[data['BEV'] == bev, :],
                      order=order, hue_order=hue_order, color='black', s=1, dodge=True)
    if be_type == 'ABE':
    ...
    Path(output_filepath + "nucleotide_percentage/").mkdir(parents=True, exist_ok=True)
    fig.savefig(output_filepath + 'nucleotide_percentage/BEV_' + output_name + '.pdf', transparent=True,
                bbox_inches="tight")
    plt.close()
...
########## EDITING EFFICIENCY FUNCTIONS ################

def run_editing_eff(old_input_file, nuc_per_filepath, output_filepath, experiment, be_type):
    '''
    This function generates the file with editing efficiency percentages for each nucleotide at each position

    INPUTS
    ------
    old_input_file: df, meta-information file
    nuc_per_filepath: str, path to folder where nucleotide percentage outputs are stored
    output_filepath: str, path to folder where outputs are stored
    experiment: str, gene being edited
    be_type: str, 'ABE' or 'CBE'

    OUTPUTS
    -------
    df: df, table listing editing efficiency percentages for each nucleotide at each position

    '''

    # check for NaN values i.e. blank rows
    if old_input_file.isnull().values.any():
        old_input_file = nb.clean_input_file(old_input_file)
    ...
    for i, r in input_file.iterrows():
        ...
        file_path = nuc_per_filepath + 'BEV_' + '_'.join(r['BEV_reps'].split(';')) + '.csv'
        print(file_path)
    ...
    df = pd.DataFrame(tuples_list,
                      columns=['Percentage', 'Position', 'Nucleotide', 'sg', 'sgRNA_sequence', 'Experiment'])
...
def make_editing_eff_plot(df, nuc, experiments, be_type, output_filepath):  # , **boxplot_kwargs, **stripplot_kwargs):
    '''
    This function

    INPUTS
    ------
    df: df, output from run_editing_eff function
    nuc: str, target nucleotide of interest (e.g. for A->G editors, nuc = 'G'; for C->T editors, nuc='T')
    experiments: list, gene(s) edited
    be_type: str, 'ABE' or 'CBE'
    output_filepath: str, path to folder where outputs are stored

    OUTPUTS
    -------
    fig, ax: editing efficiency plot with x-axis position of nucleotide being edited (e.g. 'A' for ABEs, 'C' for CBEs) and
    y-axis showing percentage editing efficiency
